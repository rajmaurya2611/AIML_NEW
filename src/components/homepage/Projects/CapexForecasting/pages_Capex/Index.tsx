...
// ✅ import the SAME Okta helper used elsewhere (adjust path if needed)
...
const Index = () => {
  ...
  // 🔐 Okta email (used as session_id)
  const [sessionEmail, setSessionEmail] = useState<string | null>(null);

  // Legacy server session support (not used as session_id anymore; kept for compatibility if needed)
  ...
  const sendMessageToBackend = async (content: string | null) => {
    setIsLoading(true);
    try {
      const payload: any = { user_id: 123 };

      if (content !== null) payload.message = content;

      // ✅ Always use Okta email as the session_id (fallback to anonymous if not available yet)
      payload.session_id = (sessionEmail || "anonymous@unknown").toLowerCase();
      ...
      // (Optional) If server returns a session_id, store it but DO NOT override our email usage.
      if (data.session_id) {
      ...
      if (data.type === "result") {
        const parts: string[] = [];
        if (data.bp_table_md) parts.push(`### My Investment (BP)\n\n${data.bp_table_md}`);
