...
import { cn } from "../../lib_Capex/utils"
...
/** Lightweight shapes so we don't depend on Recharts' exact types */
type TooltipEntry = {
  color?: string
...
type TooltipContentProps = React.HTMLAttributes<HTMLDivElement> & {
  /** Recharts passes these; we keep them optional to be tolerant */
  active?: boolean
  payload?: readonly TooltipEntry[]
  label?: unknown

  /** Recharts’ formatters – v3 uses 5 args for value formatter, 2 for label */
  formatter?: (
  ...
  /** extra props your component uses */
  hideLabel?: boolean
      ...
      if (labelFormatter) {
        return (
          <div className={cn("font-medium", labelClassName)}>
            {labelFormatter(derived ?? "", tooltipPayload)}
          </div>
        )
      }
            ...
            return (
              ...
              >
                {formatter && item.value !== undefined && item.name !== undefined ? (
                  (() => {
                    // normalize value: readonly → mutable when array
                    ...
                    {itemConfig?.icon ? (
                      ...
                      !hideIndicator && (
                        <div
                          ...
                          style={
                            {
                              "--color-bg": indicatorColor as string | undefined,
                              "--color-border": indicatorColor as string | undefined,
    ...
    return (
      ...
      >
        {payload.map((item, idx) => {
          const legendKey = `${nameKey || (item.value as string) || `legend-${idx}`}`
          const itemConfig = getPayloadConfigFromPayload(config, item as any, legendKey)
...
function getPayloadConfigFromPayload(
  ...
  if (key in (payload as any) && typeof (payload as any)[key] === "string") {
    ...
    cfgKey = (payloadPayload as any)[key] as string
  }
