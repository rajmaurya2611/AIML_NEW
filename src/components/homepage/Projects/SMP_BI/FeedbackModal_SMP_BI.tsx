...
import { useState } from "react";
import { FaStar, FaRegCommentDots } from "react-icons/fa"; // Replaced lucide-react
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const FeedbackModal = ({ isOpen, onClose }: FeedbackModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SMP_BI_BASE_URL}/feedback`, {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px] animate-scale-in">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 animate-fade-in">
            <FaRegCommentDots className="h-5 w-5 text-primary animate-pulse" />
            Share Your Feedback
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <FaStar
                    className={`smp-dialog h-8 w-8 transition-all duration-300 ${
                      star <= rating
